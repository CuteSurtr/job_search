...
/**
 * Sponsorship position stated in the posting body, or null when it says
 * nothing — which is overwhelmingly the common case. A 90-posting sample with
 * full description bodies found exactly one that mentioned the subject at all,
 * so this returns null far more often than not, and the caller falls back to
 * the curated employer record in `lib/content/sponsorship.mjs`.
 *
 * Exclusion is tested first and deliberately. "Must be authorized to work
 * without sponsorship" contains the word "sponsorship", and a positive matcher
 * run first would read that as an offer — the single worst mistake this
 * function could make, since it would send someone to apply for a job that has
 * already ruled them out.
 *
 * Everything here requires explicit wording. Nothing is inferred from an
 * employer being large, academic, or in a shortage state.
 *
 * @param {string} text
 * @returns {"documented" | "excluded" | null}
 */
export function sponsorshipFromText(text) {
  const value = String(text ?? "");

  const excluded =
    /\bnot\s+(?:be\s+)?(?:able|willing|eligible)\s+to\s+(?:provide|offer|sponsor)/i.test(value) ||
    // "unable to sponsor" and "unable to provide sponsorship" are the same
    // refusal; only the first reads as a verb.
    /\b(?:unable|not\s+able)\s+to\s+(?:sponsor\b|(?:provide|offer|take\s+over)\s+(?:visa\s+|employment\s+)?sponsorship\b)/i.test(
      value,
    ) ||
    /\bno\s+(?:visa\s+|employment\s+)?sponsorship\b/i.test(value) ||
    /\bsponsorship\s+is\s+not\s+(?:available|offered|provided)\b/i.test(value) ||
    /\bnot\s+eligible\s+for\s+(?:visa\s+|employment\s+)?sponsorship\b/i.test(value) ||
    /\bdoes\s+not\s+(?:currently\s+)?sponsor\b/i.test(value) ||
    /\bwithout\s+(?:the\s+need\s+for\s+)?(?:current\s+or\s+future\s+)?(?:visa\s+|employment\s+)?sponsorship/i.test(value);
  if (excluded) return "excluded";

  const documented =
    /\b(?:will|can|do)\s+sponsor\b/i.test(value) ||
    /\b(?:visa|immigration|employment)\s+sponsorship\s+(?:is\s+)?(?:available|offered|provided)\b/i.test(value) ||
    /\bsponsorship\s+(?:is\s+)?(?:available|offered|provided)\b/i.test(value) ||
    /\b(?:eb-?3|green\s*card)\s+sponsorship\b/i.test(value) ||
    /\bsponsor\s+(?:qualified\s+)?international\s+nurses?\b/i.test(value);
  return documented ? "documented" : null;
}
