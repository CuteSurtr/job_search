...
export function sanitizeFilters(input) {
  ...
  return {
    ...
    hideNoSponsor: source.hideNoSponsor === true,
  };
}
...
export function jobMatchesFilters(job, filters) {
  ...
  // Only ever excludes what a posting rules out in writing. A subscriber who
  // set this must not stop receiving everything merely undocumented, which is
  // almost every role.
  if (filters.hideNoSponsor && job.sponsorship === "excluded") return false;
